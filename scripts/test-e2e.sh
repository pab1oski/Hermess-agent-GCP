...
check "inferencia Vertex AI"  "KEY=\$(grep LITELLM_MASTER_KEY /home/hermess/.hermes/.env | cut -d= -f2); curl -sf -X POST http://localhost:4000/v1/chat/completions -H 'Content-Type: application/json' -H \"Authorization: Bearer \${KEY}\" -d '{\"model\":\"gemini-2.5-flash\",\"messages\":[{\"role\":\"user\",\"content\":\"say: ok\"}],\"max_tokens\":5}' | grep -q 'choices'"
